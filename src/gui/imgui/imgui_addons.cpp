...
namespace ImGui {
			  ...
			  ImVec2 graph_size)
  {
    ...
    if (scale_min == FLT_MAX || scale_max == FLT_MAX)
        ...
        for (int data_idx = 0; data_idx < num_datas; ++data_idx)
	  {
	      ...
	      {
                const float v = values[data_idx][i];
                v_min = ImMin(v_min, v);
                v_max = ImMax(v_max, v);
	      }
	  }
    ...
    if (ItemHoverable(inner_bb, 0))
        ...
        if (plot_type == ImGuiPlotType_Lines)
	  {
	      ...
	      {
                const float v0 = values[dataIdx][idx0];
                const float v1 = values[dataIdx][idx1];
    ...
    for (int data_idx = 0; data_idx < num_datas; ++data_idx)
      {
        ...
        float v0 = values[data_idx][(0 + values_offset) % values_count];
        float t0 = 0.0f;
        ...
        for (int n = 0; n < res_w; n++)
	  {
            ...
            const float v1 = values[data_idx][(v1_idx + values_offset + 1) % values_count];
  ...
  IMGUI_API void PlotMultiLines(
				...
				ImVec2 graph_size)
  {
    PlotMultiEx(ImGuiPlotType_Lines, label, num_datas, names, colors,values,
		values_count, scale_min, scale_max, graph_size);
  }
